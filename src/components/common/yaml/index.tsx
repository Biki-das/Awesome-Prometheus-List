...
import { getYaml } from "../../../utils/index";
  ...
  return (
    <div className="relative flex flex-col mt-6 px-1 last:mb-4">
      <div className=" h-[50px] flex items-center gap-x-2">
        <span className="ml-2  bg-slate-100 text-slate-500 font-bold text-[12px] rounded-full h-[40px] w-[40px] flex items-center justify-center">
          0{index + 1}
        </span>
        <div className="flex flex-col">
          <span className="text-slate-600 text-[14px] font-medium leading-[20px]">
            {rule.annotations.summary.split("(")[0]}
          </span>
          <span className="text-slate-600 text-[12px] font-medium leading-[16px]">
            {rule.annotations.description.split("\n")[0]}
          </span>
        </div>
      </div>
      <div className="flex flex-col overflow-y-scroll rounded-[4px] bg-slate-50 py-6 px-4 w-[85%] mx-auto mt-6 relative">
